...
class Fermat(object):
	...
	def fermat(self, n):
		...
		n4 = 4 * n
		c = pow(a, 2) - n4
		while not self.carre_parfait(c):
			c += aux
			a += 1
			aux += 2
		b = int(math.sqrt(c))
...
def decryptRSA(ciphertext, privatekey, etype):
	rsakey = RSA.importKey(privatekey)
	rsakey = PKCS1_OAEP.new(rsakey)
	if etype == 'base64':
		decrypted = base64.b64decode(ciphertext)
	elif etype == 'hex':
		decrypted = bytes.fromhex(ciphertext)
	elif etype == 'decimal':
...
def fermatAttack(n, e):
	...
	try:
		...
		exit()
	phi = fermat.indicatrice_euler(p, q)
	d = gmpy2.invert(e, phi)
	return d
...
def hastadAttack(n0, n1, n2, e, c0, c1, c2):
	...
	p = codecs.decode(bytes.fromhex(hex(x)[2:].replace('L', '')), 'hex').decode('utf-8')
...
def commonModulus(n, e1, e2, c1, c2):
	...
	try:
		plaintext = codecs.decode(bytes.fromhex(hex(res)[2:].replace('L', '')), "hex_codec").decode('utf-8')
