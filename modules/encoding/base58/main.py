...
def base58encode(importx, infilepath, outfilepath, inputformat, raw, exportx):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if exportx == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def base58decode(importx, infilepath, outfilepath, outputformat, raw, exportx):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
