...
def encryptBlf(blftype, importx, impfilepath, export, filepath, outputformat, ivtype, iv, passwd, raw, keyimport):
    if keyimport == 'base64':
    ...
    if blftype == 'ecb':
        ...
        cipher = Blowfish.new(key, Blowfish.MODE_OCB, iv)
    elif blftype == 'ctr':
    ...
    if export == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def decryptBlf(blftype, importx, filepath, export, expfilepath, inputformat, ivtype, iv, passwd, raw):
    if importx == 'file':
        f = open(filepath, 'r')
        raw = f.read()
        f.close()
