...
def encryptCast(casttype, importx, impfilepath, export, filepath, outputformat, ivtype, iv, passwd, raw, keyimport):
    if keyimport == 'base64':
    ...
    if casttype == 'ecb':
        ...
        cipher = CAST.new(key, CAST.MODE_OCB, iv)
    elif casttype == 'ctr':
        cipher = CAST.new(key, CAST.MODE_CTR)
    elif casttype == 'cfb':
    ...
    if export == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def decryptCast(casttype, importx, filepath, export, expfilepath, inputformat, ivtype, iv, passwd, raw):
    if importx == 'file':
        f = open(filepath, 'r')
        raw = f.read()
        f.close()
