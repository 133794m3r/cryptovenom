...
def xor(raw, key):
    xored = ''.join(chr(ord(x) ^ ord(y)) for (x,y) in zip(raw, cycle(key)))
    return xored
def encryptscrypt(importx, impfilepath, export, outfilepath, raw, keyimport, passwd, outputformat, inputformat):
    if keyimport == 'base64':
    ...
    if export == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def decryptscrypt(importx, impfilepath, export, outfilepath, raw, keyimport, passwd, inputformat, outputformat):
    if keyimport == 'base64':
        key = base64.b64decode(passwd)
    elif keyimport == 'raw':
        key = passwd 
