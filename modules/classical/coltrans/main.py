...
def coltransencode(importx, infilepath, outfilepath, inputformat, export, raw, key):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if inputformat == 'base64':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    output = ColTrans(key).encipher(iput)
    if export == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def coltransdecode(importx, infilepath, outfilepath, outputformat, export, raw, key):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
