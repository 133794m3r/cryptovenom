...
def gronsfeldencode(importx, infilepath, outfilepath, inputformat, export, raw, key):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if inputformat == 'base64':
        ...
        return False
    key = key.split(',')
    output = Gronsfeld(key).encipher(iput)
    if export == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def gronsfelddecode(importx, infilepath, outfilepath, outputformat, export, raw, key):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
        ...
        return False
    key = key.split(',')
    out = Gronsfeld(key).decipher(raw)
