...
def foursquareencode(importx, infilepath, outfilepath, inputformat, exportx, key1, key2, raw):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if inputformat == 'base64':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    output = Foursquare(key1, key2).encipher(iput)
    if exportx == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def foursquaredecode(importx, infilepath, outfilepath, outputformat, exportx, key1, key2, raw):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    out = Foursquare(key1, key2).decipher(raw)
