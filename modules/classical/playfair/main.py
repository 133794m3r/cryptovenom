...
def playfairencode(importx, infilepath, outfilepath, inputformat, export, raw, key):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if export == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def playfairdecode(importx, infilepath, outfilepath, outputformat, export, raw, key):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    out = Playfair(key=key).decipher(raw)
