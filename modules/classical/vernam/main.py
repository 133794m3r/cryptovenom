...
def VernamCipherFunction(text, key):
      ...
      for char in text:
            ...
            if ptr == len(key):
                  ptr = 0;
      return result 
def vernamencode(importx, infilepath, outfilepath, inputformat, exportx, key, raw):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if inputformat == 'base64':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    output = VernamCipherFunction(iput, key)
    if exportx == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def vernamdecode(importx, infilepath, outfilepath, outputformat, exportx, key, raw):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    out = VernamCipherFunction(raw, key)
