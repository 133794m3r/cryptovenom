...
def bifidencode(importx, infilepath, outfilepath, inputformat, exportx, raw, key, n):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if exportx == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def bifiddecode(importx, infilepath, outfilepath, outputformat, exportx, raw, key, n):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
