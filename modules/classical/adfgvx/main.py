...
def ADFGVXencode(importx, infilepath, outfilepath, inputformat, exportx, key, keyword, raw):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if inputformat == 'base64':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    output = ADFGVX(key, keyword).encipher(iput)
    if exportx == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def ADFGVXdecode(importx, infilepath, outfilepath, outputformat, exportx, key, keyword, raw):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    out = ADFGVX(key, keyword).decipher(raw)
