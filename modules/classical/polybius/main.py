...
def polybiusencode(importx, infilepath, outfilepath, inputformat, exportx, raw, key, size=5, chars=None):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
    ...
    if inputformat == 'base64':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    output = PolybiusSquare(key, size, chars).encipher(iput)
    if exportx == 'file':
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
def polybiusdecode(importx, infilepath, outfilepath, outputformat, exportx, raw, key, size=5, chars=None):
    if importx == 'file':
        f = open(infilepath, 'r')
        raw = f.read()
        f.close()
        ...
        print('\033[1;31m[-]\033[0m Unknown error.')
        return False
    out = PolybiusSquare(key,size,chars).decipher(raw)
